...
export default class SimpleWebsite {
  #server
  #nextRun = 0
  #userAgents = new Set()
  /**
  ...
  getDbInfo () {
    ...
    const totalRequestsWithoutHoneypot = db.prepare(`SELECT COUNT(*) AS totalRequestsWithoutHoneypot FROM Request_logger WHERE run_id = ? AND is_bot = 0 AND url NOT LIKE '%/honeypot%'`).get(this.#nextRun).totalRequestsWithoutHoneypot
